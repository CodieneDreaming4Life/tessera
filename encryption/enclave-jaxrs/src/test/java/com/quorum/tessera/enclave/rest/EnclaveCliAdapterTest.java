...
public class EnclaveCliAdapterTest {
    ...
    public void onSetUp() {
        ...
        Iterator<CliAdapter> adapters = ServiceLoader.load(CliAdapter.class).iterator();

        enclaveCliAdapter = StreamSupport.stream(
                Spliterators.spliteratorUnknownSize(adapters, Spliterator.ORDERED), false) //Stream complete
                .filter(EnclaveCliAdapter.class::isInstance)
                .map(EnclaveCliAdapter.class::cast)
                .findAny()
